In case you haven't heard, there's been a bit of an illness going around. No, not COVID. I'm talking about [Log4Shell](https://en.wikipedia.org/wiki/Log4Shell), possibly one of the worst zero-day vulnerabilities in the history of cybersecurity. It is endemic to Log4j2, probably the most popular logging framework for modern Java, which powers thousands of applications (including Minecraft). The power of Log4Shell cannot be understated; if a vulnerable server simply logs a specific string, remote code execution may be possible.
