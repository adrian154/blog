...
The server responds to Client Hello with connection parameters such as the TLS version it has chosen to use, the session ID, and ist own public key in response to the key_shares sent by the client.
...
Making the interactive TLS session was a bizarre experience that turned out to be surprisingly treacherous. It turns out, I have a penchant for doing things the wrong way. In this case, it involved modifying OpenSSL to log the private keys used in the handshake key exchange process and rebuilding NodeJS from source. 
