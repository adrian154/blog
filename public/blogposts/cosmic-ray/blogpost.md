*Update: I re-ran this experiment on a plane, with interesting results! Read about it [here](../cosmic-ray-2/).*
...
The original particle is called the primary cosmic ray, and it almost never makes it to the surface. Instead, when it collides with atoms in the atmosphere, a shower of secondary particles is produced. These particles are the ones which can be detected at the surface. 

To actually observe these particles, we'll be using a plain old digital camera. This is possible because the silicon detector it uses to record the intensity of light is also sensitive to many other forms of radiation! When charged particles pass through the sensor, the resulting ionization causes electrons to be ripped from the atoms within the pixels, producing charges that can be read out just like a regular image. This effect is responsible for the white speckle pattern sometimes seen in photos taken in high radiation levels.

<figure style="max-width: 500px">
    <img src="rad-meme.jpg" alt="photo of radiation source with white dots">
    <figcaption>Do not try this at home.</figcaption>
</figure>
...
Basically, our goal is to look at the value of a pixel over time and see if there are any values that are higher than expected. We can accomplish this using some basic statistics.
...
The final method I went with has three phases. It starts by computing the mean and standard deviation of each pixel; then, the z-score of every value is calculated. If the z-score exceeds some arbitrary threshold (I found that $z = 6$ gave good results), we record the event. This methodology is preeetty bad for a number of reasons&mdash;I did not use an [online algorithm](https://en.wikipedia.org/wiki/Algorithms_for_calculating_variance#Welford's_online_algorithm) to compute the variance, meaning that our code has to open every image a grand total of three times, not to mention the fact that our choice of statistics is very prone to being skewed by an outlier event&mdash;but it works pretty well despite its flaws! 

After identifying the anomalies, I grouped them into clusters by simply associating them with all other anomalies within a 16-pixel distance. We'll see how this worked out in a moment!

## Cooling

After collecting the first batch of data, I decided to do a silly little experiment. Recall that dark current increase with temperature. Therefore, we can significantly reduce the dark current level by cooling the camera. I accomplished this by freezing my camera overnight.
...
It's a pretty big reduction! Not only does the signal level go down, the distribution also becomes less dispersed, which is a product of shot noise statistics (its variance is equal to the event rate). This property can be used to determine the gain of the sensor at a given ISO.

<aside>

Because the cosmic rays produce such a strong signal, cooling is totally unnecessary for this experiment. It gave me an excuse to stick my camera in the fridge, though.

</aside>

# Results

At a confidence level of 6 &sigma;, I was left with 15,150 anomalous pixels and 2,083 clusters of at least size 2. The cosmic ray flux at sea level is about 1 muon/cm<sup>2</sup>/minute, so we should expect to have seen about 1,300 cosmic rays over the course of our experiment. This disparity can be explained by the fact that some of the particles may be of terrestrial origin (i.e. ambient radioactive decay); in addition, most particles will activate multiple pixels in the sensor.
...
This next one's interesting: there's a trail, but it's dark. I did a little digging and discovered the cause of this artifact: a particle track occurred at that spot  in another frame, and its influence on the mean was so significant that when we performed background subtraction, the imprint of that other track was left behind.
